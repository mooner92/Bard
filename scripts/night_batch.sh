#!/usr/bin/env bash
# 야간 상시 제작 (20:00~08:00) — 대본부터 완성본까지 전 공정.
#
# 설계 원칙
#  1) 재개 가능(idempotent): 각 단계는 산출물이 이미 있으면 건너뛴다.
#     중간에 죽어도 다시 켜면 이어서 진행한다.
#  2) 큐에서 줄을 지우는 시점은 "완성" 또는 "영구 실패" 뿐이다.
#     일시적 실패는 큐에 남겨 다음 밤에 재시도한다.
#  3) 08:00 이후에는 새 작품·새 클립을 시작하지 않는다.
...
NEG="rectangular box head, slab, eraser, photorealistic, photograph, 3d render, cgi, glossy, vibrant saturated colors, bright blue sky, modern clothing, anime, smooth digital art, caption, inscription, letters, signature, text, watermark, blurry, deformed"
MOTION_NEG="static, still, motionless, frozen, jittery, flickering, warping, morphing, text, watermark, blurry, distorted, deformed"

produced=0; failed=0
say "=== 야간 배치 시작 (큐 $(grep -cvE '^[[:space:]]*(#|$)' "$Q") 건) ==="
...
while work_window; do
  ...
  if [ -s "$NAR" ] && python3 -c "import json,sys; sys.exit(0 if json.load(open('$NAR'))['sentences'] else 1)" 2>/dev/null; then
    ...
    venv/bin/python scripts/write_script.py --title "$work" --facts "$facts" \
      --scenes 8 --minlen 22 --maxlen 33 --ending "$ending" --out "$NAR" >>"$LOG" 2>&1
    if [ -s "$NAR" ]; then
      # 검증 미통과여도 문장은 나온다. 야간엔 진행하고 아침에 사람이 검수한다.
      say "  ① 대본 준비됨"
    else
      say "  ① 대본 실패"; ok=0
    fi
  ...
  set -a; . ./.env; set +a
  for i in $(seq 1 "$N"); do
    W="output/tts/${work}_night_s${i}.wav"
    [ -s "$W" ] && continue
    sent=$(python3 -c "import json;print(json.load(open('$NAR'))['sentences'][$i-1])")
    esc=$(python3 -c "import sys,html;print(html.escape(sys.argv[1]))" "$sent")
    printf "<speak version='1.0' xml:lang='ko-KR'><voice name='%s'>%s</voice></speak>" \
      "$AZURE_TTS_VOICE" "$esc" > /tmp/night_ssml.xml
    curl -sf -m 60 -X POST "https://${AZURE_SPEECH_REGION}.tts.speech.microsoft.com/cognitiveservices/v1" \
      -H "Ocp-Apim-Subscription-Key: ${AZURE_SPEECH_KEY}" -H "Content-Type: application/ssml+xml" \
      -H "X-Microsoft-OutputFormat: riff-24khz-16bit-mono-pcm" -H "User-Agent: aivideo" \
      --data-binary @/tmp/night_ssml.xml -o "$W" || rm -f "$W"
  done
  ...
  for i in $(seq 1 "$N"); do
    ...
    desc=$(python3 -c "import json;print(json.load(open('$NAR'))['sentences'][$i-1])")
